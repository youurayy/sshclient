            ...
            var ff = opts.session.map(function(v) {
                if(v.op === 'writeFile') {
                    return _x(null, false, function(cb) {
                        ses.writeFile(v.path, v.body, v.mode, v.atts, _x(cb, true, function(err) {
                            if(opts.debug)
...
exports.Session = function Session(opts, conn) {
    ...
    this.writeFile = function(path, body, mode, atts, _cb) { // cb(err)
        ...
        conn.sftp(_x(cb, true, function(err, sftp) {
            ...
            sftp.open(path, mode || 'w', atts || {}, _x(cb, true, function(err, handle) {
